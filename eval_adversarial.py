...
def eval(model_name, X_train, Y_train, X_test, Y_test, cnn=False, modify=None):
    ...
    for blur_type in ["median", "gaussian", "f_gaussian", "tv_chambolle", "tv_bregman",
                      "bilateral", "nl_means"]:
        X_test_adv_blured = blur(X_test_adv, type=blur_type)
        if cnn:
            X_test_adv_blured = X_test_adv_blured.reshape(10000, 28, 28, 1)
        accuracy = th_model_eval(x, y, predictions, X_test_adv_blured, Y_test, { "batch_size" : 128 })
        print('Test accuracy on adversarial examples filtered by {}: '.format(blur_type) + str(accuracy))
